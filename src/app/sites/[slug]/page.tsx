...
export default async function SitePage({ params }: { params: { slug: string } }) {
  ...
  return h(
    ...
    site.heroImageKey &&
      h(
        "div",
        { className: "relative -mx-4 mt-3 mb-2 h-56 overflow-hidden sm:h-72 sm:rounded-lg" },
        h("img", {
    ...
    site.description &&
      h("div", {
        className: "mt-6 max-w-3xl text-ink/80",
        dangerouslySetInnerHTML: { __html: renderMarkdown(site.description) },
      }),
    site.galleryKeys.length > 0 &&
      h(
        "div",
        { className: "mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-6" },
        ...site.galleryKeys.map((key: string) =>
          h("img", {
            key,
    ...
    location &&
      h(
        ...
        h("p", { className: "px-3 py-2 text-xs text-ink/50" }, "Approximate location based on " + location + ".")
      ),
    h(
      "div",

...
export default async function SitesIndexPage({
  ...
  const where: Prisma.MiningSiteWhereInput = { status: "PUBLISHED" };
  ...
  const featuredCandidates = sites.map((site) => ({
    id: site.id,
    slug: site.slug,
    name: site.name,
    heroImageKey: site.heroImageKey,
    operatorCompanyName: site.operatorCompany?.name ?? null,
    location: [site.city, site.region, isUnresolvedCountry(site.countryCode) ? null : site.countryCode]
      .filter(Boolean)
      .join(", "),
    commodityLabel: pretty(site.commodity),
    accessTypeLabel: pretty(site.accessType),
    rosterPatterns: site.rosterPatterns,
  ...
  return h(
    ...
    hasFilters
      ...
      : sites.length === 0
        ? h("p", { className: "card mt-4 text-sm text-ink/60" }, "No sites are listed yet — check back soon.")
        : h(FeaturedCamp, { sites: featuredCandidates })
  );
}

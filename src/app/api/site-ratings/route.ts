...
export async function POST(req: NextRequest) {
  ...
  const interviewed = await prisma.application.findFirst({
        where: { candidateId: candidate.id, job: { siteId: d.siteId }, interviewedAt: { not: null } },
        select: { id: true },
  });
    if (!interviewed) {
          return NextResponse.json(
            { error: "You can only rate sites you've reached interview stage with" },
            { status: 403 }
                );
    }
  ...
  const rating = await prisma.miningSiteRating.upsert({
        where: { siteId_candidateId: { siteId: d.siteId, candidateId: candidate.id } },
        create: { siteId: d.siteId, candidateId: candidate.id, ...ratingData },
        update: ratingData,
  });

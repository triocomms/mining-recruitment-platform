...
export function MiningSiteRatingForm(props: { siteId: string; existing?: Scores | null }) {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
        ...
        setBusy(true);
        setError(null);
        const res = await fetch("/api/site-ratings", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ siteId: props.siteId, ...scores }),
        });
  ...
  return h(
    ...
    { onSubmit: submit, className: "card space-y-3" },
        h("p", { className: "label" }, props.existing ? "Update your rating" : "Rate this site"),
